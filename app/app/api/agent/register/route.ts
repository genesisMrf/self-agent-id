...
export async function POST(req: NextRequest) {
  ...
  if (!rawMode || !VALID_MODES.has(rawMode as Mode)) {
    return errorResponse(
      `Invalid mode: "${body.mode}". Valid modes: simple, verified-wallet, agent-identity, wallet-free, privy, ed25519`,
      400,
    );
  }
  ...
  const needsHumanAddress =
    mode === "simple" || mode === "agent-identity" || mode === "privy" || mode === "ed25519";
  if (
    needsHumanAddress &&
  ...
  try {
    ...
    if (mode === "simple") {
      ...
      // Ed25519: agent provides their own keypair and pre-signed challenge
      humanAddress = ethers.getAddress(body.humanAddress!);
    ...
    if (mode === "ed25519" && body.ed25519Pubkey) {
